      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <Link href="/cotacao">
          <Button className="bg-indigo-600 hover:bg-indigo-700 cursor-pointer">
            <DuckIcon className="mr-2 h-4 w-4" />
            Novo pedido
          </Button>
        </Link>
      </div>
              ...
              return (
                <div key={pedido.id} className="rounded-xl border bg-white p-6 shadow-sm">
                  <div className="mb-4 flex items-start justify-between">
                    <div>
                      <p className="text-sm text-gray-500">Pedido #{pedido.id.slice(-6).toUpperCase()}</p>
                      <p className="font-semibold text-gray-900">{pedido.itens[0]?.categoriaNome}</p>
                      <p className="text-sm text-gray-500">
                        Agendado: {formatDate(pedido.diaAgendado)} — Turno: {pedido.turno === "manha" ? "Manhã" : "Tarde"}
                      </p>
                      <p className="text-xs text-gray-400 mt-0.5">
                        Recorrência: {RECORRENCIA_LABELS[pedido.recorrencia as keyof typeof RECORRENCIA_LABELS]}
                      </p>
                    </div>
                  ...
                  <div className="relative">
                    <div className="flex items-center justify-between">
                      {ESTAGIO_ORDER.map((estagio, idx) => (
                        <div key={estagio} className="flex flex-1 flex-col items-center">
                          <div className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold ${idx <= estagioIndex ? "bg-indigo-600 text-white" : "bg-gray-200 text-gray-400"}`}>
                            {idx < estagioIndex ? <CheckCircle2 className="h-4 w-4" /> : idx + 1}
                          </div>
                    ...
                    <div className="absolute left-0 right-0 top-3 -z-10 flex items-center px-3">
                      {ESTAGIO_ORDER.slice(0, -1).map((_, idx) => (
                        <div key={idx} className={`h-0.5 flex-1 ${idx < estagioIndex ? "bg-indigo-600" : "bg-gray-200"}`} />
                      ))}
                    </div>
                  </div>
                </div>
          ...
          <div className="space-y-3">
            {pedidosConcluidos.map((pedido) => (
              <div key={pedido.id} className="rounded-xl border bg-gray-50 p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-400">#{pedido.id.slice(-6).toUpperCase()} — {formatDate(pedido.diaAgendado)}</p>
                    <p className="font-medium text-gray-700">{pedido.itens[0]?.categoriaNome}</p>
                  </div>
